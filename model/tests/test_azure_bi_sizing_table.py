"""Tests for isolated Azure baselines-included sizing table generation."""
...
def _write_metrics_csv(path: Path) -> None:
    ...
    rows = [
        ...
        ("splitwise_lut", 780.0, 690.0, 1.1304348, 20.0, -18.0),
...
def test_generate_sizing_table_dynamic_methods() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        out_csv = root / "results" / "eval_paper_baselines_included" / "azure_facility_sizing_table.csv"
        out_json = root / "results" / "eval_paper_baselines_included" / "azure_facility_sizing_table.json"
        out_tex = root / "results" / "eval_paper_baselines_included" / "azure_facility_sizing_table.tex"
        ...
        result = generate_azure_facility_sizing_table(
            ...
            out_tex=str(out_tex),
            method_order="tdp_baseline,mean_baseline,splitwise_lut,splitwise_strict,ours",
            power_factor=0.9,
        )
        ...
        assert "Splitwise (Tuned)" in tex_text
        assert "Splitwise (Strict)" in tex_text
