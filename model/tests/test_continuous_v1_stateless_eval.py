...
from model.classifiers.stateless_mean_reverting import StatelessMeanReverting
from model.scripts.continuous_v1_stateless_eval import compute_power_metrics, evaluate_from_artifacts
...
class TestContinuousV1StatelessEval(unittest.TestCase):
    def setUp(self):
        torch.manual_seed(0)
        np.random.seed(0)
    ...
    def _build_fixture(
        ...
        *,
        ...
        checkpoint_path = root / "results" / "continuous_v1_stateless" / "checkpoints" / "toy_H100_tp1_stateless_best.pt"
        checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
        model = StatelessMeanReverting(hidden_dim=8)
        torch.save(model.state_dict(), checkpoint_path)

        norm_path = root / "results" / "continuous_v1_stateless" / "norm_params" / "toy_H100_tp1.json"
        _write_json(
            norm_path,
            {
                "config_id": cfg,
        ...
        run_manifest_path = root / "results" / "continuous_v1_stateless" / "run_manifest.json"
        _write_json(
            run_manifest_path,
            {
        ...
        A_norm = np.array([0.0, 1.0, 2.0, 1.0, 0.0], dtype=np.float32)
        t_arrive_norm = np.zeros_like(A_norm, dtype=np.float32)
        x_norm = np.stack([p_prev, A_norm, t_arrive_norm], axis=1).astype(np.float32)
        active_requests = np.array([0.0, 0.0, 1.0, 2.0, 1.0, 0.0], dtype=np.float64)
        ...
        np.savez(
            ...
            active_requests=np.asarray([active_requests], dtype=object),
    ...
    def test_evaluate_from_artifacts_smoke_with_plots(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotEqual(rows[0]["overlay_plot"], "")
            self.assertNotEqual(rows[0]["mu_vs_actual_plot"], "")
            self.assertNotEqual(rows[0]["mu_function_plot"], "")
            self.assertNotEqual(rows[0]["alpha_plot"], "")
            self.assertNotEqual(rows[0]["sigma_plot"], "")
            self.assertTrue(Path(rows[0]["overlay_plot"]).exists())
            self.assertTrue(Path(rows[0]["mu_vs_actual_plot"]).exists())
            self.assertTrue(Path(rows[0]["mu_function_plot"]).exists())
            self.assertTrue(Path(rows[0]["alpha_plot"]).exists())
            self.assertTrue(Path(rows[0]["sigma_plot"]).exists())
