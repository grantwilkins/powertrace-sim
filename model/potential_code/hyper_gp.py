...
def _flatten_theta(state_dict: Dict[str, torch.Tensor], sm_comp: int) -> np.ndarray:
    """Extract 6-D hyper-parameter vector from PowerSVGP state_dict."""

    from power_gp import PowerSVGP  # local import

    Z = torch.zeros_like(state_dict["variational_strategy.inducing_points"])
    dummy = PowerSVGP(Z, sm_components=sm_comp)
    dummy.load_state_dict(state_dict, strict=False)

    prod_k, white_sk = dummy.covar_module.kernels  # 2 top-level factors
    matern_sk, sm_sk = prod_k.kernels  # factors inside product
    sigma2 = dummy.likelihood.noise.item()
    ...
    vec = np.array(
        [
            ...
            math.log(float(white_sk.outputscale)),  # RBF “white’’ scale
            math.log(float(sigma2)),  # Student-T noise
        ],
        dtype=np.float32,
    )
    return vec
