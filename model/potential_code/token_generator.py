...
class _TokenSVGP(gpytorch.models.ApproximateGP):
    def __init__(self, inducing: torch.Tensor, input_dim: int = 4):
        ...
        q_strat = gpytorch.variational.VariationalStrategy(
            self, inducing, q_dist, learn_inducing_locations=True, jitter_val=1e-2
        )
        ...
        # Student-T likelihood (will model σ² noise internally)
...
def load_model(
    path: str | Path, device: str | torch.device = "cpu", num_inducing: int = 1000
) -> _TokenSVGP:
    saved = torch.load(path, map_location=device)
    Z = torch.zeros((num_inducing, 4), dtype=torch.float32, device=device)
