...
from model.timing.iteration import HARDWARE_PROFILES, iteration_time_s, iteration_work, kv_bytes_per_token
...
def simulate_requests(
    ...
    for seq in pending:
        if seq.n_in < 0 or seq.n_out < 1 or seq.initial_context < 0:
            raise ValueError("Requests need non-negative prompt work and an output token")
        if seq.n_in == 0 and seq.initial_context == 0:
            raise ValueError("A zero-work prompt requires cached initial context")
