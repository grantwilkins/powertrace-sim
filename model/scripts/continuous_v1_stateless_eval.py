...
def _extract_norm_for_eval(norm_payload: Dict[str, object]) -> Dict[str, float]:
    ...
    out = {
        ...
        "delta_A_mean": float(norm_payload["delta_A_mean"]),
        "delta_A_std": float(norm_payload["delta_A_std"]),
    }
    if out["A_std"] <= 0.0:
...
def _load_model(
    *,
    ...
    hidden_dim = int(norm_payload.get("hidden_dim", 16))
    model = StatelessMeanReverting(hidden_dim=hidden_dim).to(device)
    try:
...
def _teacher_forced_diagnostics(
    *,
    ...
    L = int(min(len(p_prev_norm), len(A_norm), len(delta_A_norm), len(p_target_norm)))
    if L <= 0:
        raise ValueError("empty trace for diagnostics")

    p_prev = np.asarray(p_prev_norm[:L], dtype=np.float32).reshape(-1, 1)
    A = np.asarray(A_norm[:L], dtype=np.float32).reshape(-1, 1)
    dA = np.asarray(delta_A_norm[:L], dtype=np.float32).reshape(-1, 1)
    p_tgt = np.asarray(p_target_norm[:L], dtype=np.float32).reshape(-1, 1)

    A_t = torch.from_numpy(A).unsqueeze(0).to(device=device, dtype=torch.float32)
    dA_t = torch.from_numpy(dA).unsqueeze(0).to(device=device, dtype=torch.float32)
    with torch.no_grad():
        mu, alpha, log_sigma = model(A_t, dA_t)

    mu_norm = mu[0, :, 0].detach().cpu().numpy().astype(np.float64)
    alpha_np = alpha[0, :, 0].detach().cpu().numpy().astype(np.float64)
    sigma_norm = torch.exp(log_sigma[0, :, 0]).detach().cpu().numpy().astype(np.float64)
    p_prev_vec = p_prev[:, 0].astype(np.float64)
    p_target_vec = p_tgt[:, 0].astype(np.float64)
    pred_mean_norm = ((1.0 - alpha_np) * p_prev_vec) + (alpha_np * mu_norm)
...
def _plot_xy_curve(path: str, *, x: np.ndarray, y: np.ndarray, xlabel: str, ylabel: str, title: str) -> None:
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(np.asarray(x, dtype=np.float64), np.asarray(y, dtype=np.float64), linewidth=1.4)
    ax.set_title(title)
...
def _evaluate_function_curves(
    *,
    ...
    a_min = float(A_norm_min)
    a_max = float(A_norm_max)
    d_min = float(dA_norm_min)
    d_max = float(dA_norm_max)
    ...
    A_grid = np.linspace(a_min, a_max, int(max(2, num_points)), dtype=np.float32)
    dA_grid = np.linspace(d_min, d_max, int(max(2, num_points)), dtype=np.float32)

    with torch.no_grad():
        A_t = torch.from_numpy(A_grid).unsqueeze(-1).to(device=device, dtype=torch.float32)
        dA_t = torch.from_numpy(dA_grid).unsqueeze(-1).to(device=device, dtype=torch.float32)
        mu_norm = model.mu_net(A_t).squeeze(-1).detach().cpu().numpy().astype(np.float64)
        alpha = torch.sigmoid(model.alpha_net(dA_t)).squeeze(-1).detach().cpu().numpy().astype(np.float64)
        sigma_norm = torch.exp(torch.clamp(model.sigma_net(A_t), min=-6.0, max=2.0)).squeeze(-1).detach().cpu().numpy().astype(np.float64)

    mu_raw = (mu_norm * float(power_std)) + float(power_mean)
    sigma_raw = sigma_norm * float(power_std)
    return {
        "A_grid": A_grid.astype(np.float64),
        "dA_grid": dA_grid.astype(np.float64),
        "mu_raw": mu_raw,
        "alpha": alpha,
        "sigma_raw": sigma_raw,
    }
...
def _build_trace_record(
    *,
    ...
    L = int(min(len(x), len(y), len(p) - 1, max(0, len(active) - 1)))
    if L <= 0:
        raise ValueError(f"Trace {trace_idx} has no aligned points")

    delta_A_raw = (active[1 : L + 1] - active[:L]).astype(np.float64)
    return {
        ...
        "p0": float(p[0]),
        "x_norm": x[:L, :2].astype(np.float32),
        "y_norm": y[:L].astype(np.float32),
        "p_prev_norm": x[:L, 0].astype(np.float32),
        "A_norm_train": x[:L, 1].astype(np.float32),
        "delta_A_raw_train": delta_A_raw.astype(np.float32),
...
def evaluate_from_artifacts(
    *,
    run_manifest: str = "results/continuous_v1_stateless/run_manifest.json",
    ...
    out_dir: str = "results/continuous_v1_stateless/eval_metrics",
    ...
    for config_id in targets:
        ...
        try:
            ...
            with np.load(dataset_path, allow_pickle=True) as data:
                ...
                active_requests_arr = np.asarray(data["active_requests"], dtype=object)
            ...
            for idx in test_indices:
                ...
                try:
                    tr = _build_trace_record(
                        ...
                        active_requests=np.asarray(active_requests_arr[idx], dtype=np.float64),
                        dt=dt,
                    )
            ...
            a_norm_seen: List[np.ndarray] = []
            dA_norm_seen: List[np.ndarray] = []
            ...
            for tr in trace_records:
                ...
                try:
                    ...
                    A_norm_rollout = np.asarray(features_norm[:, 0], dtype=np.float64)
                    A_raw_rollout = (A_norm_rollout * float(norm_cfg["A_std"])) + float(norm_cfg["A_mean"])
                    delta_A_raw_rollout = compute_delta_active_requests(A_raw_rollout)
                    delta_A_norm_rollout = normalize_delta_active_requests(
                        delta_A_raw_rollout,
                        mean=norm_cfg["delta_A_mean"],
                        std=norm_cfg["delta_A_std"],
                    ).astype(np.float64)

                    seed_rows: List[Dict[str, object]] = []
                    pred_by_seed: Dict[int, np.ndarray] = {}
                    for seed_value in seeds:
                        pred = generate_stateless_trace(
                            model=model,
                            A_norm=A_norm_rollout,
                            delta_A_norm=delta_A_norm_rollout,
                            P_0=float(tr["p0"]),
                            config_norm={
                    ...
                    delta_A_norm_train = normalize_delta_active_requests(
                        np.asarray(tr["delta_A_raw_train"], dtype=np.float64),
                        mean=norm_cfg["delta_A_mean"],
                        std=norm_cfg["delta_A_std"],
                    ).astype(np.float64)
                    diag = _teacher_forced_diagnostics(
                        ...
                        A_norm=np.asarray(tr["A_norm_train"], dtype=np.float32),
                        delta_A_norm=delta_A_norm_train.astype(np.float32),
                    ...
                    a_norm_seen.append(np.asarray(tr["A_norm_train"], dtype=np.float64).reshape(-1))
                    dA_norm_seen.append(np.asarray(delta_A_norm_train, dtype=np.float64).reshape(-1))
            ...
            if plots and representative_gt is not None and representative_pred is not None and representative_diag is not None:
                ...
                mu_fn_path = os.path.join(plots_dir, f"{stem}_mu_function.png")
                ...
                A_vals = np.concatenate(a_norm_seen, axis=0) if a_norm_seen else np.zeros((0,), dtype=np.float64)
                dA_vals = np.concatenate(dA_norm_seen, axis=0) if dA_norm_seen else np.zeros((0,), dtype=np.float64)
                fn = _evaluate_function_curves(
                    model=model,
                    A_norm_min=float(np.min(A_vals)) if A_vals.size else -2.0,
                    A_norm_max=float(np.max(A_vals)) if A_vals.size else 3.0,
                    dA_norm_min=float(np.min(dA_vals)) if dA_vals.size else -3.0,
                    dA_norm_max=float(np.max(dA_vals)) if dA_vals.size else 3.0,
                    power_mean=norm_cfg["power_mean"],
                    power_std=norm_cfg["power_std"],
                    num_points=200,
                    device=resolved_device,
                ...
                _plot_xy_curve(
                    mu_fn_path,
                    x=np.asarray(fn["A_grid"], dtype=np.float64),
                    y=np.asarray(fn["mu_raw"], dtype=np.float64),
                    xlabel="A_t (normalized)",
                    ylabel="μ(A_t) [W]",
                    title=f"{config_id} learned μ(A_t)",
                )
                _plot_xy_curve(
                    alpha_path,
                    x=np.asarray(fn["dA_grid"], dtype=np.float64),
                    y=np.asarray(fn["alpha"], dtype=np.float64),
                    xlabel="ΔA_t (normalized)",
                    ylabel="α(ΔA_t)",
                    title=f"{config_id} learned α(ΔA_t)",
                )
                _plot_xy_curve(
                    sigma_path,
                    x=np.asarray(fn["A_grid"], dtype=np.float64),
                    y=np.asarray(fn["sigma_raw"], dtype=np.float64),
                    xlabel="A_t (normalized)",
                    ylabel="σ(A_t) [W]",
                    title=f"{config_id} learned σ(A_t)",
                )

                plot_paths = {
                    "overlay_plot": overlay_path,
                    "mu_vs_actual_plot": mu_path,
                    "mu_function_plot": mu_fn_path,
    ...
    run_manifest_payload = {
        "schema_version": "continuous-v1-stateless-eval-run-v1",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate stateless continuous v1 mean-reverting models on test traces.")
    parser.add_argument("--run-manifest", default="results/continuous_v1_stateless/run_manifest.json")
    ...
    parser.add_argument("--out-dir", default="results/continuous_v1_stateless/eval_metrics")
...
def main() -> None:
    ...
    print("[continuous_v1_stateless_eval] Done")
