...
if __name__ == "__main__":
    ...
    # classifiers = {1: load_classifier("classifier_tp1.pt", device=device)}
    ...
    for tp in dataset.state_labels:
        ...
        if args.use_gmm:
            for k in range(K):
                mu[k] = model.means_[k][0]
                sigma[k] = np.sqrt(model.covariances_[k][0])
        else:
            for k in range(K):
                mu[k] = model.cluster_centers_[k][0]
