...
from fit_power_surface import nnls_rms_scaled, run_slices
from power_surface import HARDWARE
from response_chain import apply_chain
...
def filter_design(design, run_id, dt_s: float, hardware: str,
    ...
    for _, lo, hi in run_slices(run_id):
        output[lo:hi] = apply_chain(
            design[lo:hi], dt_s, hardware, delay_s
        )
    return output
