...
def _to_item(session, tokenizer, sampler, rng, max_model_len):
    ...
    for i, (t, n_in, n_out) in enumerate(kept):
        ...
        if t.post_gap_s is not None:
            gap = float(t.post_gap_s)
        elif is_last:
            gap = 0.0
        else:
            gap = sampler.sample(t.tool_class, t.observation_tokens, rng)
...
def build_replay_plan(*, corpus="swe_smith", n_sessions, seed, tokenizer,
    ...
    if corpus == "openhands":
        ...
        sessions = swe_smith_adapter.load_swe_smith(
            n_sessions, seed, tokenizer
        )
    ...
    return agentic.from_text_transcript(
        items, prefix_cache=prefix_cache, source=corpus,
        revision=dataset_revision or str(gap_params), seed=seed,
        pack_index=pack_index, pack_count=pack_count,
    )
