...
<<<<<<< HEAD:job-scripts/gpt-oss-20b.sh
# Configuration
...
for TENSOR_PARALLEL_SIZE in ${TENSOR_PARALLEL_SIZES[@]}; do
    ...
    for ARRIVAL_RATE in ${ARRIVAL_RATES[@]}; do
        ...
        for ITERATION in $(seq 1 ${ITERATIONS}); do
            ...
            OUTPUT_PREFIX="gpt-oss-20b_tp${TENSOR_PARALLEL_SIZE}_rate${ARRIVAL_RATE}_iter${ITERATION}_${DATE_TIME}"
...
echo "Starting GPT-OSS-20B ShareGPT benchmark script"
...
start_server() {
    ...
    setsid bash serve-gpt-oss-20b.sh > "${CURRENT_SERVER_LOG}" 2>&1 &
    SERVING_PID=$!
    SERVING_PGID="$(ps -o pgid= -p "${SERVING_PID}" | tr -d ' ')"
    popd >/dev/null
...
for TENSOR_PARALLEL_SIZE in "${TENSOR_PARALLEL_SIZES[@]}"; do
    echo "Starting gpt-oss-20b benchmark at TP=${TENSOR_PARALLEL_SIZE}"
...
echo "All GPT-OSS-20B ShareGPT benchmarks complete. Output: ${OUTPUT_DIR}"
>>>>>>> grant/moe-exploration:profiling/jobs/gpt-oss-20b.sh
