...
DATASET="${SHAREGPT_DATASET_PATH:-$GROUP_DATA/ShareGPT_V3_unfiltered_cleaned_split.json}"
RUN_ROOT="${RUN_ROOT:-$ROOT/runs/gpt-oss-20b-a100-pd-${SLURM_JOB_ID:-local}}"
MODE="${1:-campaign}"
...
test -s "$DATASET" || { echo "MISSING ShareGPT dataset: $DATASET" >&2; exit 1; }
...
for cell in "${CELLS[@]}"; do
    ...
    DIR="$RUN_ROOT/$TAG"
    DONE="$DIR/.complete"
    test ! -f "$DONE" || { echo "SKIP $TAG"; continue; }
    mkdir -p "$DIR"
    ...
    "${APP[@]}" python3 profiling/client/benchmark_serving.py \
        ...
        --endpoint /v1/completions --dataset-name sharegpt --dataset-path "$DATASET" \
        --request-rate "$RATE" --num-prompts "$PROMPTS" --seed 0 \
    ...
    test -s "$DIR/power.csv" && test -s "$DIR/engine_prefill.csv" \
        && test -s "$DIR/engine_decode.csv"
    touch "$DONE"
    echo "COMPLETE $TAG prompts=$PROMPTS"
done
