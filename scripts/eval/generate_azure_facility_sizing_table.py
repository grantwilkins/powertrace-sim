...
def _normalize_method_order(method_order: Sequence[str] | str) -> List[str]:
    ...
    for value in values:
        ...
        if value not in allowed:
            raise ValueError(f"Unsupported method '{value}'. Allowed: {sorted(allowed)}")
        if value not in out:
            out.append(value)
    if not out:
    ...
    with open(metrics_csv, "r", newline="") as f:
        ...
        for row in reader:
            ...
            out[kind] = {
                ...
                "ramp_max_down_kw_per_step": safe_float(row.get("ramp_max_down_kw_per_step", ""), "ramp_max_down_kw_per_step"),
            }

    missing_kinds = [kind for kind in method_order if kind not in out]
    if missing_kinds:
...
def _build_latex_table(
    *,
    ...
    headers = [TRACE_KIND_HEADER.get(method, method) for method in method_order]
    align = "l" + ("c" * len(method_order))
    head_line = " & ".join([r"\textbf{Metric}"] + [rf"\textbf{{{header}}}" for header in headers]) + r" \\"
    ...
    for row in rows:
        values = [row["metric"]] + [row[method] for method in method_order]
    ...
    write_json(
        out_json,
        {
            ...
            "method_metrics_15min": {
                method: {metric: float(value) for metric, value in values.items()}
                for method, values in method_metrics.items()
            },
            "table_rows": [dict(row) for row in table_rows],
        },
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate the infrastructure sizing table from Azure facility metrics."
    )
