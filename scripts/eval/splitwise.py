#!/usr/bin/env python3
"""Splitwise-style LUT comparator used by the maintained facility study."""
...
DEFAULT_NODE_GPU_TDP_W = {"A100": 3300.0, "H100": 4600.0}
...
def _resolve_hardware(config: Mapping[str, object]) -> str:
    hardware = str(config.get("hardware", "")).strip().upper()
    if hardware in DEFAULT_NODE_GPU_TDP_W:
        return hardware
    match = CONFIG_HW_RE.match(str(config.get("config_id", "")).strip())
    if match:
        return match.group(1)
    raise ValueError("unable to resolve hardware")


def _resolve_tp(config: Mapping[str, object]) -> int:
    if "tp" in config and int(config["tp"]) > 0:
        return int(config["tp"])
    match = CONFIG_HW_TP_RE.match(str(config.get("config_id", "")).strip())
    if match and int(match.group(2)) > 0:
        return int(match.group(2))
    raise ValueError("unable to resolve TP")
