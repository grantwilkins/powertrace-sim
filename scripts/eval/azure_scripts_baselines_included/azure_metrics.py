#!/usr/bin/env python3
"""
Isolated Azure pipeline: compute facility metrics for Ours + Splitwise + constants.
"""
...
NON_CONSTANT_METHODS = {"ours", "splitwise_lut", "splitwise_strict"}
...
def _load_method_site_arrays(aggregated_root: str, method: str) -> Dict[float, np.ndarray]:
    ...
    for res_s, fname in RESOLUTION_FILE_MAP.items():
        out[res_s] = _load_array(os.path.join(method_dir, fname))
    return out
...
def _normalize_trace_kinds(trace_kinds: Sequence[str] | str) -> List[str]:
    ...
    for k in kinds:
        if not k:
            continue
        if k not in allowed:
            raise ValueError(f"Unsupported trace_kind '{k}'. Allowed: {sorted(allowed)}")
        if k not in out:
            out.append(k)
    if not out:
...
def compute_azure_facility_metrics(
    *,
    ...
    for method, site_it_by_res in method_site_it_w.items():
        node_dir = os.path.join(node_traces_root, method)
    ...
    site_traces_rows: List[Dict[str, object]] = []
    ...
    for resolution_s in (0.25, 1.0, 60.0, 900.0):
        ...
        for trace_kind in selected_kinds:
            ...
            if float(resolution_s) == 900.0:
                ...
                for i, v in enumerate(values_kw):
                    site_traces_rows.append(
                        {
                            "trace_kind": str(trace_kind),
                            "bin_idx": int(i),
                            "hour": float(hours[i]),
                            "power_kw": float(v),
                            "power_mw": float(v / 1000.0),
                        }
                    )
    ...
    with open(site_traces_15min_csv, "w", newline="") as f:
        ...
        writer.writeheader()
        writer.writerows(site_traces_rows)
    ...
    summary = {
        ...
        "n_rows_site_traces": int(len(site_traces_rows)),
        "layout": {
        ...
        "diversity_by_method": {k: float(v) for k, v in diversity_by_method.items()},
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Compute isolated Azure facility metrics with Splitwise baselines.")
    ...
    print("=" * 72)
    print("Azure Facility Metrics (Baselines Included)")
    print("=" * 72)
