#!/usr/bin/env python3
"""
Isolated Azure pipeline: generate facility-scale figures with Splitwise baselines.
"""
...
def _normalize_trace_kinds(trace_kinds: Sequence[str] | str) -> List[str]:
    ...
    for v in values:
        if not v:
            continue
        if v not in allowed:
            raise ValueError(f"Unsupported trace_kind '{v}'. Allowed: {sorted(allowed)}")
        if v not in out:
            out.append(v)
    if not out:
...
def _load_arrival_rate_binned(
    ...
    with open(parsed_requests_csv, "r", newline="") as f:
        ...
        for row_idx, row in enumerate(reader, start=2):
            t = safe_float(row.get("arrival_time", ""), "arrival_time")
            if t < 0.0:
                raise ValueError(f"Negative arrival_time at row {row_idx}: {t}")
            b = int(np.floor(t / float(bin_seconds)))
            if b < 0 or b >= n_bins:
                raise ValueError(f"arrival_time out of [0, {day_seconds}) at row {row_idx}: {t}")
            counts[b] += 1
            n_rows += 1
...
def _load_metrics_15min_rows(metrics_csv: str, trace_kinds: Sequence[str]) -> Dict[str, Dict[str, float]]:
    ...
    missing_kinds = [k for k in trace_kinds if k not in out]
    if missing_kinds:
...
def _load_ldc_15min(ldc_csv: str, trace_kinds: Sequence[str]) -> Dict[str, np.ndarray]:
    ...
    grouped: Dict[str, List[tuple[int, float, float]]] = {k: [] for k in trace_kinds}
    ...
    for trace_kind in trace_kinds:
        ...
        out[f"{trace_kind}_fraction"] = np.asarray([x[1] for x in rows_sorted], dtype=np.float64)
        out[f"{trace_kind}_power_mw"] = np.asarray([x[2] / 1000.0 for x in rows_sorted], dtype=np.float64)
    return out
...
def _load_site_traces_15min(site_traces_15min_csv: str, trace_kinds: Sequence[str]) -> Dict[str, np.ndarray]:
    ...
    for kind in trace_kinds:
        ...
        rows_sorted = sorted(rows, key=lambda x: int(x[0]))
        out[f"{kind}_hours"] = np.asarray([x[1] for x in rows_sorted], dtype=np.float64)
        out[f"{kind}_power_mw"] = np.asarray([x[2] for x in rows_sorted], dtype=np.float64)
    return out
...
def _compute_splitwise_vs_ours_stats(metrics_rows_15min: Mapping[str, Mapping[str, float]]) -> Dict[str, float]:
    ...
    ours_avg = float(metrics_rows_15min["ours"]["avg_kw"])
    out: Dict[str, float] = {}
    for key in ("splitwise_lut", "splitwise_strict"):
        if key not in metrics_rows_15min:
            continue
        out[f"{key}_over_peak_pct_vs_ours"] = float(
            (float(metrics_rows_15min[key]["peak_kw"]) - ours_peak) / ours_peak * 100.0
        )
        out[f"{key}_over_avg_pct_vs_ours"] = float(
            (float(metrics_rows_15min[key]["avg_kw"]) - ours_avg) / ours_avg * 100.0
        )
    return out
...
def _plot_figure_1_diurnal_overlay(
    *,
    ...
    l1 = ax1.plot(power_hours, site_mw, color=COLOR_DARK, linewidth=1.8, label="Site Power (15-min)")[0]
...
def _plot_figure_2_baseline_comparison(
    *,
    ...
    if "splitwise_lut_over_peak_pct_vs_ours" in splitwise_vs_ours:
        ann_lines.append(
            f"SW tuned vs Ours peak: {float(splitwise_vs_ours['splitwise_lut_over_peak_pct_vs_ours']):.1f}%"
        )
    if "splitwise_strict_over_peak_pct_vs_ours" in splitwise_vs_ours:
        ann_lines.append(
            f"SW strict vs Ours peak: {float(splitwise_vs_ours['splitwise_strict_over_peak_pct_vs_ours']):.1f}%"
        )
    ...
    *,
    ...
    for idx, method in enumerate(trace_kinds):
        ...
        for b in bars:
            ...
            ax.text(b.get_x() + b.get_width() / 2.0, h + ypad, f"{h:.2f}", ha="center", va="bottom", fontsize=7)
...
def generate_azure_figures(
    *,
    ...
    sizing_metrics = _compute_sizing_metrics_from_rows(metrics_rows, trace_order, power_resolution_seconds=900)
    p99_mw_by_method = {k: float(metrics_rows[k]["ldc_p99_kw"]) / 1000.0 for k in trace_order}

    paths = {
        "figure_1": str(Path(out_dir) / "azure_bi_figure_1_diurnal_profile.pdf"),
        "figure_2": str(Path(out_dir) / "azure_bi_figure_2_baseline_comparison_15min.pdf"),
        "figure_3": str(Path(out_dir) / "azure_bi_figure_3_load_duration_curve.pdf"),
        "figure_4": str(Path(out_dir) / "azure_bi_figure_4_rack_heatmap.pdf"),
        "figure_5": str(Path(out_dir) / "azure_bi_figure_5_sizing_metrics.pdf"),
        "manifest": str(Path(out_dir) / "azure_bi_figure_manifest.json"),
    }
    ...
    manifest: Dict[str, object] = {
        "schema_version": "azure-figures-baselines-included-v1",
        "inputs": {
        ...
        "derived_metrics": {
            ...
            "splitwise_vs_ours": {k: float(v) for k, v in splitwise_vs_ours.items()},
            "p99_mw_by_method": {k: float(v) for k, v in p99_mw_by_method.items()},
            "sizing_metrics": {k: {kk: float(vv) for kk, vv in sizing_metrics[k].items()} for k in trace_order},
        },
        "output_paths": {
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate isolated Azure figures from baselines-included outputs.")
    ...
    if bool(args.dry_run):
        print("[azure_bi_figures] Dry run complete")
    else:
        print("[azure_bi_figures] Done")
